...
static bool ImGui_ImplFreeType_LoaderInit(ImFontAtlas* atlas) {
  ...
  FT_Library_Version(bd->Library, &ver_linked_major, &ver_linked_minor,
                     &ver_linked_patch);
  snprintf(GImGuiFreeTypeLoaderName, sizeof(GImGuiFreeTypeLoaderName),
           "FreeType (%d.%d.%d; %d.%d.%d)", FREETYPE_MAJOR, FREETYPE_MINOR,
           FREETYPE_PATCH, ver_linked_major, ver_linked_minor,
           ver_linked_patch);
  ...
  if (src->MergeMode == false) {
    ...
    baked->Ascent = (float)FT_CEIL(metrics.ascender) *
                    scale; // The pixel extents above the baseline in pixels
                           // (typically positive).
    baked->Descent =
        (float)FT_CEIL(metrics.descender) *
...
const ImFontLoader* ImGuiFreeType::GetFontLoader() {
  ...
  loader->Name =
      GImGuiFreeTypeLoaderName; // Initially "FreeType" then updated during the
                                // call to ImGui_ImplFreeType_LoaderInit()
